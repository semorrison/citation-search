...
//  Future(Search.query("warming up ..."))
  ...
  def apply(req: HttpRequest): Future[HttpResponse] = {
    ...
    val json = results.map({ case (c, q) => f"""   { MRNumber: ${c.MRNumber}, title: "${c.title}", authors: "${c.authors}", cite: "${c.cite}", url: "${c.url}", ${c.pdf.map(p => f"""pdf: "$p", """).getOrElse("")}${c.free.map(f => f"""pdf: "$f", """).getOrElse("")}score: $q } """.replaceAllLiterally("\\", "\\\\") }).mkString(s"""{ query: "$query",\n  results: [\n""", ",\n", "  ]\n}")
    callback match {
      case Some(c) => {
